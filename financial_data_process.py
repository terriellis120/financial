...
def find_swinglowhigh(date_data) :
    ...
    plt.plot(timestamp[peaks], close_data[peaks], "o")
    plt.plot(timestamp[valleys], close_data[valleys], "o")
    
    plt.savefig('./plots/'+cur_date+'.png')  
    plt.close(fig)
    
    return [(timestamp[peaks], close_data[peaks]), (timestamp[valleys], close_data[valleys]) ]



def find_trendlines(x, y) :
